...
| T06 | 力量程最大值 | **>25N**（填入） | — | HKVT Fz=15N, 安全过载 400%(=60N)。**>25N 超出 Fz 量程**；建议方案 C Hybrid 或换向量程更大的力传感器 | 📄+⚠️ | datasheet | 量程匹配须甲方确认 |
...
**T06 量程冲突提示**：甲方填入力量程最大值 **>25N**，而 HKVT-M3A Fz 量程仅 **15N**（安全过载 400% = 60N）。若 >25N 为硬性需求，单颗 HKVT 无法满足，建议方案 C（Hybrid，补大量程力传感器）或更换向量程传感器。**此项须甲方确认是否为硬约束**。
